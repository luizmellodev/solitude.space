// Vamos diminuir a velocidade da chuva
...
export default function RainEffect() {
  ...
  const detectObstacles = () => {
    ...
    // Select all relevant elements that should block raindrops
    const elements = document.querySelectorAll(
      'h1, h2, p, button, .card, .backdrop-blur-md, [role="tablist"], [role="tab"], .playlist-item, .ambient-item, #track-info, #player-controls, #sidebar, #productivity-button',
    )

    elements.forEach((element) => {
      const rect = element.getBoundingClientRect()

      // Only add visible elements with some size
  ...
  useEffect(() => {
    ...
    const createRaindrop = () => {
      ...
      // Reduced speed for more gentle rain
      const speed = Math.random() * 6 + 4 // Reduced from 15+10 to 6+4
